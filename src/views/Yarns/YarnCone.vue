<template>
    <div class="tw-relative tw-w-full tw-h-full">
        <div
            v-if="loading"
            class="tw-absolute tw-inset-0 tw-z-10 tw-flex tw-items-center tw-justify-center tw-bg-white/80">
            <v-progress-circular
                color="primary"
...
<script lang="ts" setup>
    ...
    const coneHeight = 16;
    const coneRadiusBottom = 3.25;
    const coneRadiusTop = 2.0;
        ...
        for (let row = 0; row < numRows; row++) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                for (let j = 0; j <= segments; j++) {
                    ...
                    const bundleCenterPoint = new THREE.Vector3(currentRadius * Math.cos(currentAngle), currentYPos, currentRadius * Math.sin(currentAngle));
